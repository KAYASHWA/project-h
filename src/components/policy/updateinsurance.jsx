...
class UpdatePolicy extends React.Component {
  ...
  componentDidMount() {
    axios
      .get(
        `http://localhost:8090/api/getPolicy/${this.props.match.params.pNum}`
      )
      .then((res) => {
  ...
  handleSubmit = (event) => {
    ...
    axios
      .post("http://localhost:8090/api/addPolicy", this.state.insurances)
      .then((res) => {
        console.log(res.data);
        alert("updated policy " + this.state.insurances.pName + " successfully");
